...
@export var spin_interval: float = 0.15   
var _spin_dirs: Array = [
	Vector2.DOWN,
	Vector2.LEFT,
	Vector2.UP,
	Vector2.RIGHT
...
func _update_movement(delta: float) -> void:
	...
	if t >= 1.0:
		...
		if _is_spin_tile(global_position):
			_start_spin()
			return
...
func _is_spin_tile(position: Vector2) -> bool:
	if ground_layer == null:
		return false
	...
	if tile_data == null:
		return false
	if tile_data.has_custom_data("spin"):
		return bool(tile_data.get_custom_data("spin"))
...
func _start_spin() -> void:
	...
	_spin_timer = 0.0
	_spin_index = 0
	var dir: Vector2 = _spin_dirs[_spin_index]
...
func _update_spin(delta: float) -> void:
	...
	if _spin_timer >= spin_interval:
		...
		_spin_index = (_spin_index + 1) % _spin_dirs.size()
		var dir: Vector2 = _spin_dirs[_spin_index]
		_last_facing_dir = dir
		_play_idle(dir)
